...
**Objectives:** Oral capsaicin studies often rely on static endpoints, leaving temporal response patterns and symptom heterogeneity underexamined. We combined trajectory analysis, symptom-region mapping, and Rome IV-informed categorization to characterize heterogeneity in oral capsaicin-induced visceral pain.
...
**Results:** The cohort mean trajectory peaked at minute 4 (mean VAS 4.22) and then declined. Median threshold-defined onset was 1.0 minute; median relief was not reached within 20 minutes. Dynamic time warping clustering identified delayed-peak (n=58), early-sustained (n=84), and late-rising (n=74) phenotypes (silhouette score 0.299). Abdominal distension was the most frequent symptom (154/216, 71.3%), and the right hypochondrium carried the largest regional burden (140/216, 64.8%; weighted degree 368). Rome IV-informed mapping clustered around biliary pain-like, bloating/distension-like, and irritable bowel syndrome-like patterns. Early-window phenotype classification reached accuracy 0.828 by 8 minutes and 0.902 with the full 20-minute trajectory. Logistic regression achieved balanced accuracy of 0.654 and macro F1 of 0.626 for next-step VAS direction prediction; baseline-feature phenotype prediction remained modest (best primary accuracy 0.601).

**Conclusions:** Oral capsaicin elicited heterogeneous visceral pain trajectories with reproducible temporal patterns. Trajectory, symptom-region, and Rome IV-informed analyses identified symptom constellations relevant to disorders of gut-brain interaction. Static summaries captured only part of this structure. Early-window classification and short-horizon prediction may help future studies track dynamic phenotypes and intervention-related trajectory change.
...
- Early-window and next-step prediction analyses captured short-horizon response information; baseline-only phenotype prediction remained modest.
...
DTW-based clustering identified three temporal patterns: delayed-peak responders (n=58), early-sustained responders (n=84), and late-rising responders (n=74) (Figure 1B). The average silhouette score was 0.299, indicating moderate separation among overlapping classes. Fuzzy clustering and shapelet analysis followed the same pattern but did not support discrete subtypes.
...
Rome IV-informed mapping clustered around biliary pain-like (57/216, 26.4%), functional abdominal bloating/distension-like (35/216, 16.2%), and irritable bowel syndrome-like (25/216, 11.6%) patterns (Figure 2D). These categories describe symptom-pattern alignment, not formal DGBI diagnoses.

### Symptom-region network structure

The symptom-region frequency matrix was spatially non-uniform (Figure 2C). The largest co-occurrence edge linked abdominal distension with the right hypochondrium (weight=110), and high-intensity cells remained concentrated in the right hypochondrium and hypogastrium. The right hypochondrium had the largest weighted degree (368), followed by the hypogastrium (267).

The tripartite network centered on the right hypochondrium and hypogastrium and connected most often to biliary pain-like, bloating/distension-like, and irritable bowel syndrome-like alignments. The grouped Sankey summary indicated that all three temporal phenotypes shared overlapping symptom-region hubs, especially abdominal distension, nausea, abdominal pain, the right hypochondrium, and the hypogastrium, while `No Match` remained a common terminal Rome IV-informed state. Because the analysis expanded participant-level co-occurrence patterns, these findings describe recurrent combinations rather than one-to-one anatomical localization.
...
Oral capsaicin responses were temporally patterned and heterogeneous. The cohort-level VAS trajectory was biphasic, with rapid onset and slower resolution (Figure 1A,D). Participant-level responses separated into three partly overlapping temporal phenotypes instead of a single curve (Figure 1B). Symptom analyses added spatial and symptom-construct context: abdominal distension was common, the right hypochondrium carried the largest network burden, and Rome IV-informed mapping linked experimental symptom profiles to biliary pain-like, bloating/distension-like, and irritable bowel syndrome-like constructs (Figure 2A-D). Static intensity summaries provide an incomplete account of visceral pain across time, symptom quality, and perceived anatomical distribution [1-4,7,8].

The temporal findings favor response-shape analysis alongside conventional static summaries. Median onset occurred within 1 minute, the main change point clustered around minute 5, and median relief was not reached during the 20-minute observation period (Figure 1C,D). Peak VAS and AUC capture these features only partly, although such endpoints remain common in experimental pain studies and pain assessment frameworks [24-27]. In this model, onset, persistence, and recovery behaved as related phases of the same response [26,27].

The fixed administration protocol, early onset, clustered change-point timing, and recoverable phenotype structure gave the model clear cohort-level temporal landmarks.

The observed early rise followed by slower decline is physiologically plausible in light of prior capsaicin and TRPV1 literature. Capsaicin activates TRPV1-expressing visceral afferents and can evoke burning, cramping, nausea, and distension-like sensations in the upper gastrointestinal tract [12,14-18]. Prior work also indicates that TRPV1 signaling changes over time, with downstream modulation and calcium-dependent desensitization shaping responses after initial activation [15,16,20-22]. This study did not test receptor-level mechanisms. The combination of rapid onset, delayed recovery, and between-subject variability is consistent with a time-dependent sensory process rather than a brief impulse-like event [17,24,25].

The clustering results argue against treating oral capsaicin pain as a single canonical curve. DTW preserved shape information despite between-participant variation in peak timing, and fuzzy clustering showed blurred phenotype boundaries (Figure 1B). Investigators should interpret the three phenotypes as recurring response patterns rather than fixed biological subtypes. That interpretation agrees with prior work in which pain and DGBI-related symptom data contain structured heterogeneity rather than one homogeneous presentation [2,7,8,26,31]. The labels delayed-peak, early-sustained, and late-rising are descriptive. DTW clustering, fuzzy assignment, early-window classification, and shapelet motifs all recovered the same structure, which makes an analytic artifact less likely. Investigators still need independent replication and stronger physiological anchoring before treating these patterns as disease entities.

Symptom-region mapping showed that capsaicin-induced sensations were unevenly distributed across anatomical regions. In Figure 2A-C, abdominal distension, nausea, and abdominal pain clustered in the right hypochondrium and hypogastrium rather than spreading evenly across the abdominal map. The supplementary Sankey view showed that the temporal phenotypes did not branch into fully separate symptom repertoires; they shared symptom-region hubs and a small set of frequent Rome IV-informed endpoints, with a large `No Match` remainder that reflects the conservative rule-based mapping. This spatial concentration may inform region-aware symptom assessment, although readers should not treat it as an anatomical map of nociceptive origin. Network-based symptom analyses describe how complaints cluster across participants; they do not identify a single underlying organ source [29,30,37-39].

The Rome IV-informed categories require cautious interpretation. The mapping assessed symptom overlap with Rome IV constructs; it did not diagnose DGBIs or incorporate the chronicity requirements central to Rome IV classification [7,8,10,11]. The biliary pain-like, bloating/distension-like, and irritable bowel syndrome-like patterns in Figure 2D indicate that an acute provocation can generate symptom constellations that resemble clinically recognized constructs at the level of phenomenology. The model can be used to study symptom generation, but it remains distinct from patient diagnosis [7,8,10].

The prediction analyses separated trajectory information from pre-exposure information. Early VAS windows improved phenotype classification, indicating that trajectory shape became informative within the first minutes after exposure (Figure 3A). By 8 minutes, classification accuracy reached 0.828. By 20 minutes, phenotype membership was largely recoverable from the observed trajectory history, as expected because the clustering outcome was trajectory-derived. Baseline demographic and questionnaire-derived exposure-history and symptom features, together with ECG, EGG, and ECG-EGG coupling features, showed only modest discrimination (Figure 3B,D). In this experimental setting, the evolving pain signal carried more phenotype-specific information than the pre-exposure feature set. The observed signal anticipated near-term VAS direction and identified an emerging phenotype before the full trajectory unfolded. The analysis did not model treatment-related improvement, but the same framework can be extended once interventional trajectories are available.

This study has three main limitations. First, VAS remains a subjective self-report measure, and early terminations introduced censoring and informative missingness that may distort late-trajectory summaries. Second, the phenotype structure was analytically derived and therefore sensitive to preprocessing and modeling choices, with fuzzy assignments indicating boundary cases rather than discrete groups. Third, the Rome IV-informed mapping and ECG/EGG feature panel were limited proxies, supporting descriptive rather than diagnostic or biological interpretation. Future work should quantify the impact of missingness, test robustness across analytic pipelines and independent cohorts, and add stronger multimodal physiological measures for mechanistic anchoring and symptom mapping.

Within this protocol, the oral capsaicin model provided a controlled setting for studying heterogeneous visceral pain dynamics. Trajectory analysis, symptom-region structure, and Rome IV-informed mapping identified DGBI-relevant symptom patterns in addition to pain intensity. These response patterns may guide future studies that test dynamic phenotyping, short-horizon forecasting, and prospective prediction of intervention-related improvement. Investigators still need between-session stability data, external replication, and stronger biological anchoring before using the model as a surrogate for clinical disease [1,2,7,8,31].
