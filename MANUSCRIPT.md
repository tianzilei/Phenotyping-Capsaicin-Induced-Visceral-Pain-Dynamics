...
**Conclusions:** Within this standardized protocol, oral capsaicin produced heterogeneous visceral pain trajectories with identifiable temporal structure and recurring response phenotypes. Integrating trajectory, symptom-region, and Rome IV-informed analyses revealed disorders-of-gut-brain-interaction-relevant symptom constellations beyond those captured by static summaries. Early-window and short-horizon prediction results suggest that the model may support dynamic phenotyping and future studies of intervention-related trajectory change.
...
- Standardized provocation yielded concentrated temporal landmarks and recoverable phenotype structure within the study protocol.
...
Prediction from baseline demographic and questionnaire-derived exposure-history and symptom features, together with ECG, EGG, and ECG-EGG coupling features, remained modest (Figure 3B). In the primary model set, histogram-based gradient boosting achieved the highest mean accuracy (0.601), while random forest achieved the highest mean balanced accuracy (0.610). The complementary confusion matrix also showed only partial separation across the three phenotypes (Figure 3D). The measured baseline and physiological features therefore provided limited discrimination of phenotype membership.
...
The temporal findings support response-shape analysis alongside conventional static summaries. Median onset occurred within 1 minute, the main change point clustered around minute 5, and median relief was not reached during the 20-minute observation period (Figure 1C,D). Peak VAS and AUC capture these features only partly, even though such endpoints remain common in experimental pain studies and broader pain assessment frameworks [24-27]. These results indicate that the oral capsaicin model can distinguish onset, persistence, and recovery as separate phases of the same response [26,27].

Taken together, the fixed administration protocol, early onset, concentrated change-point timing, and recoverable phenotype structure indicate that the model provides an experimentally controllable provocation paradigm with well-defined cohort-level temporal landmarks.
...
Detailed supplementary methods and results are provided in the Supplementary Appendix, including Supplementary Methods S1-S3, Figures S1-S5, and Tables S1-S4, with Tables S2b and S2c reported as supplementary network extensions. These materials provide expanded early-window classification details, ECG/EGG preprocessing and feature-derivation procedures, baseline-plus-physiology summaries, shapelet motifs, symptom-region and Rome IV-informed network summaries, short-term direction-prediction metrics, complementary feature-importance diagnostics, and adverse-event tabulations that extend, rather than duplicate, the main-text figures and Table 1.
